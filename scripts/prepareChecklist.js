...
const patchFile = (path, replacements) => {
  ...
  for (const [from, to] of replacements) {
    if (!source.includes(from)) {
      throw new Error(`Checklist build patch did not find expected source in ${path}: ${from.slice(0, 120)}`);
    }
...
console.log('Checklist input/notes patch applied successfully.');
