...
export const SupabaseLoginScreen: React.FC<Props> = ({ config, onLogin }) => {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!profile.active) {
        await import('../lib/authService').then(({ signOutSupabase }) => signOutSupabase());
        window.alert('هذا الحساب غير مفعل. تواصل مع المطور.');
        return;
      }

      const isDeveloper = profile.role === 'Developer' || profile.role === 'developer' || profile.position === 'System Admin';
      onLogin(isDeveloper ? 'admin' : 'user', user.id);
    } catch (error) {
      ...
      <div className="w-full max-w-md rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xl p-5 sm:p-7">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">تسجيل الدخول الآمن</p>
          </div>
        </div>
        ...
        <form onSubmit={submit} className="space-y-4">
          <label className="block">
            ...
            <div className="mt-1.5 relative">
              ...
              <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" autoComplete="username" dir="ltr" className="w-full h-11 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 pr-10 pl-3 outline-none focus:ring-2 focus:ring-rose-500" placeholder="name@example.com" disabled={busy} />
            </div>
          </label>
          ...
          <label className="block">
            ...
            <div className="mt-1.5 relative">
              ...
              <input value={password} onChange={(e) => setPassword(e.target.value)} type="password" autoComplete="current-password" dir="ltr" className="w-full h-11 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 pr-10 pl-3 outline-none focus:ring-2 focus:ring-rose-500" placeholder="••••••••" disabled={busy} />
            </div>
          </label>

          <button disabled={busy} className="w-full h-11 rounded-xl bg-rose-600 hover:bg-rose-700 disabled:opacity-60 text-white font-black flex items-center justify-center gap-2">
            <ShieldCheck className="w-4 h-4" /> {busy ? 'جارٍ التحقق...' : 'دخول'}
          </button>
        </form>

        <p className="mt-4 text-[11px] text-slate-500 text-center">يتم التحقق من الحساب والصلاحيات من قاعدة البيانات.</p>
      </div>
    </div>
  );
};
