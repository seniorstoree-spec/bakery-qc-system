...
const AppShell:React.FC=()=>{
  ...
  const loadAuthProfile=async(userId:string)=>{const { data, error } = await supabase.from('users').select('id,name,position,role,permissions,active').eq('auth_user_id',userId).eq('active',true).single();if(error||!data){await supabase.auth.signOut();setAuthUserId(null);throw new Error('حساب Auth غير مربوط بمستخدم نشط داخل النظام.')}setCurrentUserProfile(toUserProfile(data));setAuthUserId(userId)};
  useEffect(()=>{let mounted=true;const boot=async()=>{try{const current=await getCurrentAuthProfile();if(!mounted)return;if(current){setCurrentUserProfile(toUserProfile(current.profile));setAuthUserId(current.user.id);const remote=await syncAdminConfigFromSupabase();if(mounted&&remote)setConfig(remote)}}catch(error){console.warn('Auth bootstrap failed',error)}finally{if(mounted)setBooting(false)}};void boot();const {data:{subscription}}=supabase.auth.onAuthStateChange((_event,session)=>{if(!mounted)return;if(session?.user){void loadAuthProfile(session.user.id).catch(()=>undefined)}else{setAuthUserId(null)}});return()=>{mounted=false;subscription.unsubscribe()};},[]);
