...
export const ProductReleaseModule: React.FC = () => {
  ...
  const toggleCondition=(key:keyof typeof form.mandatoryConditions)=>{
    ...
    updateForm({mandatoryConditions:{...form.mandatoryConditions,[key]:!form.mandatoryConditions[key]}});
  };
  const allConditionsMet=Object.values(form.mandatoryConditions).every(Boolean);
  const handleQaSignOff=()=>{if(!currentUser.permissions.canApproveRelease)return alert('تنبيه: يتطلب اعتماد إذن الإفراج النهائي صلاحيات مدير الجودة / رئيس قسم المراقبة.');if(!allConditionsMet)return alert('لا يمكن اعتماد الإفراج: يجب استيفاء كافة الشروط الـ 5 الإلزامية أولاً.');const now=new Date().toISOString();updateForm({decision:'approved',qaReleaseOfficerName:currentUser.name,qaReleaseOfficerSignature:currentUser.name,qaReleaseOfficerTimestamp:now});};
  const handleStorekeeperSignOff=()=>{if(form.decision!=='approved')return alert('تنبيه: لا يمكن لأمين المخزن استلام التشغيلة قبل اعتماد إذن الإفراج من إدارة الجودة.');const now=new Date().toISOString();updateForm({storekeeperName:currentUser.name,storekeeperSignature:currentUser.name,storekeeperTimestamp:now});};
  const handleArchiveReport=async()=>{if(archiving)return;setArchiving(true);setArchiveError('');setArchiveSuccess(false);try{const daily=await getOrCreateDailyReport(activeDate);updateForm({dailyReportId:daily.id});await archiveReport(daily.id);setArchiveSuccess(true);}catch(error){setArchiveError(error instanceof Error?error.message:'تعذر تخزين التقرير في الأرشيف');}finally{setArchiving(false);}};
  return <div className="space-y-6 animate-fadeIn pb-12">
    <HeaderBanner title="إذن الإفراج عن المنتج التام (Finished Product Release Approval)" subtitle="التحقق من المعايير الخمسة الإلزامية والاعتماد الإلكتروني للإفراج والتخزين والتسليم" docCode="QC-IS-FM-01-16" revNo="0" date={activeDate}/>
    ...
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-emerald-200 p-5 shadow-sm"><div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4"><div><div className="flex items-center gap-2 text-emerald-700 font-extrabold"><Archive className="w-5 h-5"/><span>تخزين التقرير بالكامل في الأرشيف</span></div><p className="text-xs text-slate-500 mt-1">يتم إنشاء/استرجاع تقرير اليوم ثم حفظ المعرف وربطه بإذن الإفراج قبل الأرشفة.</p></div><button type="button" onClick={handleArchiveReport} disabled={archiving} className="min-w-56 px-5 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold flex items-center justify-center gap-2 disabled:opacity-60">{archiving?<Loader2 className="w-4 h-4 animate-spin"/>:<Archive className="w-4 h-4"/>}<span>{archiving?'جارٍ التخزين...':'تخزين التقرير في الأرشيف'}</span></button></div></div>
    <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border shadow-sm"><div className="flex items-center gap-2"><ShieldCheck className="w-5 h-5 text-emerald-600"/><h2 className="font-extrabold">إذن الإفراج عن المنتج التام</h2></div><div className="mt-4 text-sm">التاريخ: <b>{activeDate}</b> — الحالة: <b>{form.decision}</b> — عدد الأصناف: <b>{form.products.length.toLocaleString('en-US')}</b></div></div>
