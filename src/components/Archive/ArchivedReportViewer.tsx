...
const fields: Record<string,string> = {
  ...
  quantity:'الكمية', criterion:'المعيار', category:'التصنيف', createdAt:'تاريخ الإنشاء', created_at:'تاريخ الإنشاء', updatedAt:'تاريخ التحديث', updated_at:'تاريخ التحديث',
  closedAt:'تاريخ الإغلاق', closed_at:'تاريخ الإغلاق', archivedAt:'تاريخ الأرشفة', archived_at:'تاريخ الأرشفة', status:'الحالة', complianceStatus:'حالة المطابقة', compliance_status:'حالة المطابقة',
  name:'الاسم', code:'الكود', time:'الوقت', stage:'المرحلة', morningShift:'وردية الصباح', eveningShift:'وردية المساء', mandatoryConditions:'الشروط الإلزامية',
  rawMaterialsCompliant:'مطابقة الخامات', ccpOprpReportsCompliant:'مطابقة CCP/OPRP', labAnalysisCompliant:'مطابقة التحاليل المعملية', labelAndPackagingCompliant:'مطابقة البطاقات والتعبئة',
  customerRequirementsCompliant:'مطابقة متطلبات العميل', qaReleaseOfficerName:'مسؤول الإفراج', storekeeperName:'أمين المخزن', products:'المنتجات', reason:'سبب عدم المطابقة', savedAt:'وقت التسجيل', saved_at:'وقت التسجيل',
  verifiedBy:'تم التحقق بواسطة', verified_by:'تم التحقق بواسطة', responsiblePerson:'المسؤول', responsible_person:'المسؤول', correctiveAction:'الإجراء التصحيحي', corrective_action:'الإجراء التصحيحي',
  machineCode:'كود الماكينة', machine_code:'كود الماكينة', fryerCode:'كود القلاية', fryer_code:'كود القلاية', coreTemperature:'درجة حرارة المركز', temperature:'درجة الحرارة', duration:'المدة',
  isCompliant:'مطابق', is_compliant:'مطابق', packagingQuality:'جودة التعبئة', packaging_quality:'جودة التعبئة', afterStatus:'الحالة بعد الإجراء', beforeStatus:'الحالة قبل الإجراء', responsible:'المسؤول', score:'الدرجة', comments:'التعليقات',
  sampleCode:'كود العينة', sample_code:'كود العينة', inspectorName:'اسم المفتش', inspector_name:'اسم المفتش', reportId:'رقم التقرير', dailyReportId:'رقم التقرير اليومي', daily_report_id:'رقم التقرير اليومي',
  sectionsCompleted:'الأقسام المكتملة', sections_completed:'الأقسام المكتملة', totalSections:'إجمالي الأقسام', total_sections:'إجمالي الأقسام', createdBy:'أنشأه', created_by:'أنشأه', verified:'تم التحقق', verifiedAt:'وقت التحقق', verified_at:'وقت التحقق',
  defectType:'نوع العيب', defect_type:'نوع العيب', description:'وصف العيب', action:'الإجراء', severity:'درجة الخطورة', frequency:'التكرار', shift:'الوردية', criticalDeviation:'انحراف حرج', critical_deviation:'انحراف حرج', defectSummary:'تفاصيل العيوب', defect_summary:'تفاصيل العيوب',
  doughWeight:'وزن العجين', doughWeightMin:'الحد الأدنى لوزن العجين', doughWeightMax:'الحد الأقصى لوزن العجين', doughWeightTime:'وقت وزن العجين',
  bakedWeight:'وزن بعد التسوية', bakedWeightMin:'الحد الأدنى بعد التسوية', bakedWeightMax:'الحد الأقصى بعد التسوية', bakedWeightTime:'وقت وزن بعد التسوية',
  filledWeight:'وزن بعد الحشو', filledWeightMin:'الحد الأدنى بعد الحشو', filledWeightMax:'الحد الأقصى بعد الحشو', filledWeightTime:'وقت وزن بعد الحشو',
  finishedWeight:'وزن بعد الفنش', finishedWeightMin:'الحد الأدنى بعد الفنش', finishedWeightMax:'الحد الأقصى بعد الفنش', finishedWeightTime:'وقت وزن بعد الفنش',
  allowableLimit3Pct:'حد العيوب 3%', allowableLimit5Pct:'حد العيوب 5%', criticalReleaseLimitPct:'الحد الحرج للإفراج', feCriticalMm:'حد FE الحرج (مم)', nfeCriticalMm:'حد NFe الحرج (مم)', ssCriticalMm:'حد SS الحرج (مم)',
  matrixMinScore:'الحد الأدنى للتقييم الحسي', matrixMaxScore:'الحد الأقصى للتقييم الحسي'
};

const values: Record<string,string> = {
  archived:'مؤرشف', open:'مفتوح', closed:'مغلق', pending:'قيد الانتظار', pending_review:'قيد المراجعة', pass:'ناجح', passed:'مطابق', fail:'غير مطابق', failed:'غير مطابق', compliant:'مطابق', noncompliant:'غير مطابق', non_compliant:'غير مطابق', 'non-compliant':'غير مطابق',
  approved:'معتمد', rejected:'مرفوض', conforming:'مطابق', nonconforming:'غير مطابق', start:'بداية', mid:'منتصف', middle:'منتصف', end:'نهاية', final:'نهائي', bakery:'قسم المخبوزات', daily_product:'منتج يومي', dailyProduct:'منتج يومي', yes:'نعم', no:'لا', true:'نعم', false:'لا'
};
...
const display=(v:unknown):string=>{
  ...
  if(typeof v==='string'){
    ...
    return v.replace(/\bnon[_ -]?compliant\b/gi,'غير مطابق').replace(/\bcompliant\b/gi,'مطابق').replace(/\barchived\b/gi,'مؤرشف').replace(/\bopen\b/gi,'مفتوح').replace(/\bclosed\b/gi,'مغلق').replace(/\bpending(?:[_ -]?review)?\b/gi,'قيد الانتظار').replace(/\bapproved\b/gi,'معتمد').replace(/\brejected\b/gi,'مرفوض').replace(/\bstart\b/gi,'بداية').replace(/\bmid(dle)?\b/gi,'منتصف').replace(/\bend\b/gi,'نهاية').replace(/\bpass(ed)?\b/gi,'مطابق').replace(/\bfail(ed)?\b/gi,'غير مطابق');
  }
  return String(v);
};
...
const valueAt=(row:Record<string,unknown>,key:string):unknown=>{
  ...
  const camel=key.replace(/_([a-z])/g,(_,c)=>String(c).toUpperCase()); if(Object.prototype.hasOwnProperty.call(row,camel)) return row[camel];
  const snake=key.replace(/[A-Z]/g,m=>`_${m.toLowerCase()}`); if(Object.prototype.hasOwnProperty.call(row,snake)) return row[snake];
  return undefined;
};

const defectFields: Array<[string,string]> = [
  ['oversize','زيادة الحجم'],['undersize','نقص الحجم'],['overweight','زيادة الوزن'],['underweight','نقص الوزن'],['dark_color','لون داكن'],['light_color','لون فاتح'],['burnt_parts','أجزاء محروقة'],['deflated_product','منتج منكمش'],['gaps_in_pieces','فراغات بين القطع'],['dry_product','منتج جاف'],['doughy_product','منتج عجيني'],['non_laminated','عدم التوريق'],['bitter_taste','طعم مر'],['rancid_taste','طعم زنخ'],['filling_leakage','تسريب الحشوة'],['excess_filling','زيادة الحشوة'],['insufficient_filling','نقص الحشوة'],['no_filling','بدون حشوة'],['heavy_texture','قوام ثقيل'],['light_texture','قوام خفيف'],['excess_glaze','زيادة التغطية'],['insufficient_glaze','نقص التغطية'],['surface_spots','بقع سطحية'],['surface_peeling','تقشر السطح'],['surface_cracks','تشقق السطح'],['foreign_matters','مواد غريبة'],['expiry_date_defect','عيب تاريخ الصلاحية'],['sealing_defect','عيب الغلق'],['printing_defect','عيب الطباعة'],['undesired_smell','رائحة غير مرغوبة']
];
const summarizeDefect=(row:Record<string,unknown>):string=>{const details=defectFields.filter(([key])=>Number(valueAt(row,key)??0)>0).map(([key,name])=>`${name}: ${display(valueAt(row,key))}`);return details.length?details.join(' • '):'تم تسجيل الفحص بدون عيوب كمية محددة';};
...
const renderSectionData=(key:string,data:unknown)=>{
  ...
  if(key==='defects'){
    ...
    return <SimpleTable rows={normalized} columns={['date','product_name','stage','time','required_production_qty','sample_size','allowable_limit_3_pct','allowable_limit_5_pct','defect_summary','status','critical_deviation']}/>;
  }
  ...
  if(key==='sensoryEvaluations') return <SimpleTable rows={rows} columns={['date','productName','sampleNumber','sampleType','isVegan','colorScore','tasteScore','aromaScore','textureScore','overallImpressionScore','overallRating','matrixMinScore','matrixMaxScore','inspectorName']}/>;
  if(key==='ncr') return <SimpleTable rows={rows} columns={['date','status','productName','productionQty','detectedDefects','defectiveQty','defectPercentage','rootCause','correctiveAction','signee']}/>;
  if(key==='productWeights') return <SimpleTable rows={rows} columns={['date','productName','time','doughWeight','doughWeightMin','doughWeightMax','doughWeightTime','bakedWeight','bakedWeightMin','bakedWeightMax','bakedWeightTime','filledWeight','filledWeightMin','filledWeightMax','filledWeightTime','finishedWeight','finishedWeightMin','finishedWeightMax','finishedWeightTime','isCompliant']}/>;
  if(key==='metalDetector') return <SimpleTable rows={rows} columns={['date','time','machineCode','feStatus','feCriticalMm','nfeStatus','nfeCriticalMm','ssStatus','ssCriticalMm','isCompliant','responsiblePerson','correctiveAction','verifiedBy']}/>;
...
export const ArchivedReportViewer:React.FC<{reportDate:string;status:string;sectionsCompleted?:number;totalSections?:number;snapshot:Record<string,unknown>;engineerName:string}>=({reportDate,status,sectionsCompleted,totalSections,snapshot,engineerName})=>{
  ...
  return <div className="space-y-5"><div className="no-print grid grid-cols-2 lg:grid-cols-4 gap-3"><div className="rounded-2xl border bg-white p-4"><div className="text-xs text-slate-500">الأقسام المسجلة</div><div className="text-2xl font-black mt-1">{sections.length}</div></div><div className="rounded-2xl border bg-white p-4"><div className="text-xs text-slate-500">إجمالي السجلات</div><div className="text-2xl font-black mt-1">{rowCount}</div></div><div className="rounded-2xl border bg-white p-4"><div className="text-xs text-slate-500">العيوب</div><div className="text-2xl font-black mt-1">{defectRows}</div></div><div className="rounded-2xl border bg-white p-4"><div className="text-xs text-slate-500">حالات غير مطابقة</div><div className="text-2xl font-black mt-1 text-rose-600">{nonCompliant}</div></div></div><div className="no-print flex flex-wrap items-center justify-between gap-3 rounded-2xl border bg-white p-4"><div><div className="font-black text-slate-800">التقرير اليومي للجودة — {reportDate}</div><div className="text-sm text-slate-500">المهندس المسجل: {engineerName||'—'} · الكود: {reportCode}</div></div><button onClick={()=>print()} className="inline-flex items-center gap-2 px-4 py-3 rounded-xl bg-rose-600 text-white font-black shadow-sm hover:bg-rose-700"><FileDown className="w-5 h-5"/>تصدير التقرير كامل PDF</button></div><div id="archived-report-print-area" className="space-y-5">{sections.map(([key,value])=><Section key={key} sectionKey={key} title={labels[key]||label(key)} data={value} onPrint={()=>print(key)}/>)}</div></div>;
};