...
export async function listArchiveMonths(): Promise<ArchiveMonth[]> {
  ...
  for (const row of data ?? []) {
    ...
    const [y,m] = String(row.report_date).split('-');
    const year = Number(y), month = Number(m), key = `${year}-${month}`;
...
export async function archiveReport(reportId:string) {
  const {data,error}=await supabase.from('daily_quality_reports').update({status:'archived', archived_at:new Date().toISOString()}).eq('id',reportId).select().single();
  if(error) throw error;
  return data as DailyQualityReport;
}
