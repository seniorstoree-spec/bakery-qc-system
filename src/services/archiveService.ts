...
export async function getOrCreateDailyReport(reportDate:string):Promise<DailyQualityReport>{
  ...
  if(createError){
    if(createError.code==='23505'){
      const {data:concurrent}=await supabase.from('daily_quality_reports').select('*').eq('report_date',reportDate).eq('department','bakery').single();
...
export async function listArchiveReports(year:number,month:number):Promise<ArchivedReportDetails[]>{
  ...
  const {data,error}=await supabase.from('daily_quality_reports').select('*').eq('department','bakery').eq('status','archived').gte('report_date',start).lt('report_date',end).order('report_date',{ascending:false});
  if(error)throw error;return(data??[]).map(mapArchivedReport);
}
...
export async function getArchiveReport(reportId:string):Promise<ArchivedReportDetails>{
  ...
  if(error)throw error;return mapArchivedReport(data);
}
...
export async function saveArchiveReport(reportId:string,patch:Partial<DailyQualityReport>):Promise<DailyQualityReport>{
  ...
  if(error)throw error;return mapDailyReport(data);
}
...
export async function archiveReport(reportId?:string):Promise<ArchivedReportDetails>{
  ...
  // Freeze a complete, date-based snapshot before changing the master record to archived.
  // The snapshot is what makes an archived report self-contained and prevents later edits from changing history.
